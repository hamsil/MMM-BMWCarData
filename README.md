...
- translation
- reverse geocoding
- map tiles _(sometimes towns are named in English nevertheless...)_
...
```js
{
  ...
  config: {
    ...
    topics: [ // at least one topic to display
...
```js
{
  ...
  position: "top_center",
  config: {
    width: "600px", // optional, but default 100% is too narrow for top_center
  }
}
```

The configuration of my own MagicMirror² with a MINI and a BMW currently is as follows:
...
| `capabilities-{vin}.json`      | Once at startup             | Vehicle capability flags from the BMW CarData REST API — which features the car supports. Stored for reference; not used by the module itself. _Well, that's the theory, in practice I always get error 403 when calling the CarData API for capabilities and thus a warning in the log :-(_ |
...
| `image`                    | Vehicle image from the BMW CarData REST API — fetched once and cached as `data/image-{vin}.jpg`. Rendered as a scaled image that fills the cell width (use `span` to make it wider). _Again this is the theory, in practice I always get error 403 when calling the CarData API for the image and thus a warning in the log :-(_ |
...
| Provider                            | `tileUrl`                                              |
| ----------------------------------- | ------------------------------------------------------ |
| **CartoDB Dark Matter** _(default)_ | `/MMM-BMWCarDataInfo/tile/carto-dark/{z}/{x}/{y}.png`  |
